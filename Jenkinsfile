pipeline {
    ...
    environment {
        IMAGE_NAME = "yash5090/bb-movie-char"
    ...
    stages {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash509/DevDecOps-BBCHAR-DSPY-WBSTE-DEP.git'
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop bb-movie-char | true"
                sh "docker rm bb-movie-char | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh "docker run -d --name bb-movie-char -p 3000:3000 ${IMAGE_NAME}:${TAG}" 
                //}
            }
        }
        ...
        stage('Switch Traffic Between Blue & Green Environment') {
            ...
            steps {
                script {
                    ...
                    withKubeConfig(caCertificate: '', clusterName: '', contextName: '', credentialsId: 'k8s', namespace: '', restrictKubeConfigAccess: false, serverUrl: '') {
                        sh '''
                            kubectl patch service bb-movie-char-service -p "{\\"spec\\": {\\"selector\\": {\\"app\\": \\"bb-movie-char\\", \\"version\\": \\"''' + newEnv + '''\\"}}}"
                        '''
                    }
